  ...
  \draw[->] (p)--(y);
   ...
   \begin{pgfonlayer}{background}
    ...
    \begin{scope}[xshift = 8cm, node distance = 1cm]
        ...
        \node[below of = m, node distance = 1.5cm] (h)
        {$h_{ij} = (432 - |hi_{ij} - m_i|) / 432$};
        
        \node[below of = h] (l)
        {$l_{ij} = (432 - |lo_{ij} - m_i|) / 432$};
  
        \node[below of = l] (p)
        {$p_{ij} = \frac{\mbox{exp}\left\{ c \cdot h_{ij} \right\}}
        {\mbox{exp}\left \{c \cdot h_{ij} \right \}+ \mbox{exp} \left \{ c \cdot l_{ij} \right \}}$};
